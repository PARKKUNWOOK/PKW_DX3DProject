...
void CubeMapEditor::CreateWall()
{
	...
	Cube* wall = new Cube({ 1, 1, 1 });
	wall->SetLocalPosition(pos);
	wall->GetMaterial()->GetData()->diffuse = { 0, 1, 0, 1 };
	wall->Update();
...
{
	Vector3 startPos = Vector3(mapSize.x * -0.5f, 0.0f, mapSize.y * -0.5f);
	Vector3 pos = startPos + Vector3(selectTile.x + 0.5f, 0.8f, selectTile.y + 0.5f);
	...
	Cube* startPosition = new Cube({ 1, 1, 1 });
	startPosition->SetLocalPosition(pos);
	startPosition->GetMaterial()->GetData()->diffuse = { 0, 1, 0, 1 };
...
void CubeMapEditor::CreatePreview()
{
	...
	wall->GetMaterial()->GetData()->emissive = { 0, 1, 0, 0.5f };
	wall->SetActive(true);

	previews[Wall] = wall;

	Cone* startPosition = new Cone(0.3f, 0.5f);
	startPosition->SetLocalRotation(0, XM_PI, 0);
